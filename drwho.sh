...
function f_certStatus {
    ...
    cat drwho_tmp/ssl_sum2.txt | tr -d '"' | sed 's/, Inc//' | sed 's/ = /: /g' | sed 's/_/ - /g' | tr -d '(' | tr -d ')' | 
    sed 's/^ *//' > drwho_tmp/ssl.txt
    ...
    issuer_cn=`echo "$issuer" | grep -w 'CN:' | sed 's/^ *//'`
    ...
    echo -e -n "${B}Expires:${D}     " 
    cat drwho_tmp/ssl.txt | grep -i -m 1 'after' | cut -d ' ' -f 4- 
    echo -e "\n${B}Subject:     ${D}$subject_cn $subject_country" 
    echo -e "\n${B}Issuer:      ${D}$issuer_org $issuer_country\n             $issuer_cn" 
    echo -e "\n\n${BDim}TLS-Vers.:  ${D}$protocol" 
    echo -e "\n${BDim}Cipher:     ${D}$cipher" 
    echo -e "\n${BDim}Signature:  ${D}$algo"  
    echo -e "\n\n${B}Public Key${D}\n" 
    cat drwho_tmp/ssl.txt | grep -i -m 1 -A 2 'public' | sed '/Info/d' | sed 's/Public-Key:/Key Length:/g'
}

#************ certificate chain *************
function f_certChain {
if [ ! -f "drwho_tmp/ssl.txt" ]; then
    timeout 3 openssl s_client -connect $target:443 -brief 2>drwho_tmp/ssl_sum2.txt
    echo | timeout 3 openssl s_client -connect $target:443 2> drwho_tmp/status.txt -status >> drwho_tmp/status.txt
    echo | timeout 3 openssl s_client -connect $target:443 2>>drwho_tmp/ssl_sum2.txt | openssl x509 -text -enddate >> drwho_tmp/ssl_sum2.txt
else 
    echo -e "${B}Certificate Chain ${D}\n\n"   
    echo -e "${B}Certificate Authority (depth: 2)${D}\n"  
    root_ca=`cat drwho_tmp/ssl.txt | grep -i -m 1 -A 1 'depth=2' | sed 's/depth=2//' | sed 's/,/\n/g' | sed '/L:/d'  | sed '/ST:/d' | 
    sed '/postal/d' | sed '/street:/d' | sed 's/^ *//' | sed 's/verify/\nverify/' | sed 's/return:/return: /' | sed 's/C:/C:   /' | 
    sed 's/CN:/CN:  /' |   sed 's/O:/O:  /' | sed 's/OU:/OU:  /'`
    ...
    echo "$root_ca_org $root_ca_country" 
    echo "$root_ca_cn" 
    echo ''
    echo "$root_ca" | grep -i -w 'verify' | sed 's/return:/return: /'  
    #********* issuer of leaf certifics
    echo -e "\n\n${B}Certificate Authority (depth: 1)${D}\n"  
    ...
    echo "$ca_org $ca_country"  
    echo "$ca_cn"                
    echo ''
    echo "$ca" | grep -i -w 'verify' | sed 's/return:/return: /' 
    #********* leaf certificate
    echo -e "\n\n${B}Leaf Certificate Issued to:${D}\n"  
    subject=`cat drwho_tmp/ssl.txt | grep -m 1 -w 'Subject:' | sed 's/, Inc//' | sed 's/,/\n/g' | sed 's/Subject://g'` 
    subject_cn=`echo "$subject" | grep -w 'CN:' | sed 's/^ *//'`
    echo "$subject_cn"  
    cat drwho_tmp/ssl.txt | grep  -i -w -m 1 -A 1 'Constraints:' | sed "/constraints:/d" | sed "/Constraints:/d" | sed 's/CA:/\nIS CA: /'  
fi
}
...
function f_showCerts {
    echo -e "\n" >>  $HOME/drwho_results/$file.txt
    echo -e "${B}$target Certificates${D}\n"  
    echo -e -n " ${BDim}Local Time & Date:${D}  "
    date
    echo -e "\n"
    timeout 3 openssl s_client -connect $target:443 -showcerts  
    f_solidLong
    echo -e "\n${B}$target Public Key${T}\n\n"  
    timeout 3 openssl s_client -connect $target:443 2>>/dev/null | openssl x509 -pubkey -noout  
    echo -e "${D}"
}

#************************** ssl/tls related stuff going into text file *********************************
function f_saveCert_Info {
    subject=`cat drwho_tmp/ssl.txt | grep -m 1 -w 'Subject:' | sed 's/, Inc//' | sed 's/,/\n/g' | sed 's/Subject://g'`
    subject_org=`echo "$subject" | grep -w 'O:' | sed 's/^ *//' | sed 's/O:/O: /'`
    subject_country=`echo "$subject" | grep -w 'C:' | tr -d ' ' | sed 's/C:/\/ /'  `
    subject_cn=`echo "$subject" | grep -w 'CN:' | sed 's/^ *//'`
    ...
    echo -e "          Certificate Information"      >> $HOME/drwho_results/$file.txt
    echo -e "    -------------------------------------\n" >> $HOME/drwho_results/$file.txt
    echo -e "Certificate Status\n\n" >> $HOME/drwho_results/$file.txt
    echo 'Valid until: ' >> $HOME/drwho_results/$file.txt
    cat drwho_tmp/ssl.txt | grep -i -m 1 'after' | cut -d ' ' -f 4- >> $HOME/drwho_results/$file.txt
    echo -e "\nSubject:     $subject_cn $subject_country"                          >> $HOME/drwho_results/$file.txt   
    echo -e "\nIssuer:      $issuer_org $issuer_country\n             $issuer_cn"  >> $HOME/drwho_results/$file.txt   
    echo -e "\n\nTLS-Vers.:  $protocol"                                            >> $HOME/drwho_results/$file.txt   
    echo -e "\nCipher:     $cipher"                                                >> $HOME/drwho_results/$file.txt   
    echo -e "\nSignature:  $algo"                                                  >> $HOME/drwho_results/$file.txt   
    echo -e "\n\nPublic Key \n"                                                    >> $HOME/drwho_results/$file.txt   
    cat drwho_tmp/ssl.txt | grep -i -m 1 -A 2 'public' | sed '/Info/d' | sed 's/Public-Key:/Key Length:/g'  >> $HOME/drwho_results/$file.txt   
    root_ca=`cat drwho_tmp/ssl.txt | grep -i -m 1 -A 1 'depth=2' | sed 's/depth=2//' | sed 's/,/\n/g' | sed '/L:/d'  | sed '/ST:/d' | sed '/postal/d' | 
    sed '/street:/d' | sed 's/^ *//' | sed 's/verify/\nverify/' | sed 's/return:/return: /' | sed 's/C:/C:   /' | sed 's/CN:/CN:  /' |   sed 's/O:/O:  /' | 
    sed 's/OU:/OU:  /'`
    root_ca_org=`echo "$root_ca" | grep -w 'O:' | sed 's/^ *//' | sed 's/O:/O: /'` 
    root_ca_country=`echo "$root_ca" | grep -w 'C:' | tr -d ' ' | sed 's/C:/\/ /'`
    root_ca_cn=`echo "$root_ca" | grep -w 'CN:' | sed 's/^ *//' `
    ...
    ca_cn=`echo "$ca" | grep -w 'CN:' | sed 's/^ *//'`
    echo -e "\n________________________________________________________________\n" >> $HOME/drwho_results/$file.txt
    echo -e "Certificate Chain\n"                                                  >> $HOME/drwho_results/$file.txt   
    echo -e "\nCertificate Authority (depth: 2)\n"                                 >> $HOME/drwho_results/$file.txt
    echo -e "$root_ca_org $root_ca_country\n$root_ca_cn"                           >> $HOME/drwho_results/$file.txt
    echo "$root_ca" | grep -i -w 'verify' | sed 's/return:/return: /'              >> $HOME/drwho_results/$file.txt
    echo -e "\n\nCertificate Authority (depth: 1)\n"                               >> $HOME/drwho_results/$file.txt
    echo -e "$ca_org $ca_country\n$ca_cn"                                          >> $HOME/drwho_results/$file.txt
    echo "$ca" | grep -i -w 'verify' | sed 's/return:/return: /'                   >> $HOME/drwho_results/$file.txt
    echo -e "\n\nLeaf Certificate Issued to\n"                                     >> $HOME/drwho_results/$file.txt
    echo "$subject_cn"                                                             >> $HOME/drwho_results/$file.txt
}
...
function f_timeStamp {
    echo -e "\n"
    echo -e -n " ${BDim}Header Timestamp:${D}    "
    curl -s -I -L --max-time 3 $target > drwho_tmp/headers.txt
    cat drwho_tmp/headers.txt | grep -m 1 Date | cut -d ' ' -f 2-
    echo ''
    echo -e -n " ${BDim}Current Local Time:${D}  "
    date
    echo -e "\n"
}

function f_serverStatus {
    echo ''
    cat drwho_tmp/headers.txt | sed -n '/HTTP/p;/[Ss]erver/p'
}
...
function f_guessCMS {
    ...
    echo -e -n "${B}CMS:${D}  "  
    echo -e "CMS" >> $HOME/drwho_results/$file.txt
    cat drwho_tmp/cms.txt | grep -i -o -F -econtao -edrupal -ejoomla -eliferay -etypo3 -ewordpress | tr '[a-z]' '[A-Z]' | sort | uniq  | 
...
echo -e "\n________________________________________________________________\n" >> $HOME/drwho_results/$file.txt
    echo -e "    -------------------"           >> $HOME/drwho_results/$file.txt
    echo -e "           whois"                  >> $HOME/drwho_results/$file.txt
    echo -e "    -------------------\n"         >> $HOME/drwho_results/$file.txt
    echo -e "Host Whois Summary\n"              >> $HOME/drwho_results/$file.txt
    whois $target > drwho_tmp/host-whois.txt
    cat drwho_tmp/host-whois.txt | sed '/^#/d' | sed '/^%/d' | sed '/icann.org/d' | sed '/NOTICE/d' | sed '/reflect/d' | sed '/Fax:/d' | 
    sed '/Fax Ext:/d' | sed '/unsolicited/d' | sed '/HKIRC-Accredited/d' | sed /'how to'/d | sed '/queried/d' | sed '/Bundled/d' | 
    sed '/Registry Domain ID:/d' | sed 's/^ *//' | sed "/^[[:space:]]*$/d"  > drwho_tmp/whois.txt
...
function f_menuDomain {
    ...
    echo ''
    echo -e "  ${W}11)${B}   hail mary"
    echo -e "  ${W}12)${B}   server response times & tracepath"
    echo -e "  ${W}13)${B}   A,AAAA,MX,NS,PTR,SOA & TXT records"
    echo -e "  ${W}14)${B}   whois lookup options"
    echo -e "  ${W}15)${B}   geolocation options (ext.API)"
    echo -e "  ${W}16)${B}   CMS, website title, description &\n        social media links"
    echo -e "  ${W}17)${B}   dump HTTP headers"
    echo -e "  ${W}18)${B}   dump robots.txt"
    echo -e "  ${W}19)${B}   link dump"
    echo -e "  ${W}20)${B}   certificate information & -files"
    echo -e "  ${W}21)${B}   zone transfer check"
    echo -e "  ${W}22)${B}   address block reverse host search"
    echo -e "  ${W}23)${B}   subdomain enumeration (ext.API)"
    echo -e "  ${W}24)${B}   reverse IP lookup (ext.API)"
f_solidShort
}
...
f_textfileSeparator
echo -e "    -----------------" >> $HOME/drwho_results/$file.txt
echo -e "        Hail Mary"      >> $HOME/drwho_results/$file.txt
echo -e "    -----------------\n" >> $HOME/drwho_results/$file.txt
...
lynx -accept_all_cookies -dump -listonly -nonumbers www.$target | grep -F -econtact -ediscord -ekontakt -econtatto -eimpressum -eetsy -efacebook -egithub -einstagram -elinkedin -epinterest -ereddit -etwitter -exing -eyoutube | sed '/sport/d' |  sed '/program/d'  | sort | uniq | fmt -w 60 -s | 
tee -a $HOME/drwho_results/$file.txt
f_solidLong
echo -e "${B}Host Geolocation & AS ${D}\n"
f_textfileSeparator
echo -e "Host Geolocation & AS\n" >> $HOME/drwho_results/$file.txt
echo -e " ${BDim}$target - $host_ip${D}\n"
address=`echo $host_ip`
f_geoIP
f_solidLong
echo -e "${B}Start of Authority PNS Geolocation & AS${D}\n"
echo -e "\n\nStart of Authority PNS Geolocation & AS\n" >> $HOME/drwho_results/$file.txt
...
f_geoIP
f_solidLong
#****** mail- & name server IP4 addresses  ******
...
f_solidLong
#**** certificate information & target  geolocation
f_certStatus
f_solidLong
f_certChain 
f_saveCert_Info
...
f_drwho
f_solidLong
echo -e "${B}SOA Record Reverse whois Summary${D}\n"
echo -e "\n\nSOA Record Reverse whois Summary\n"  >> $HOME/drwho_results/$file.txt
echo -e " ${BDim}$soa_url - $soa_ip${D}\n"
...
;;
13)
#********** 13) DNS Records - A, AAAA, MX, NS, PTR, SOA, TXT **********
f_solidLong
f_textfileSeparator
echo -e "    ------------------------"      >> $HOME/drwho_results/$file.txt
...
f_textfileSeparator
echo -e "    ------------------------"          >> $HOME/drwho_results/$file.txt
echo -e "        MX reverse whois"              >> $HOME/drwho_results/$file.txt
echo -e "    ------------------------\n"        >> $HOME/drwho_results/$file.txt
...
f_textfileSeparator
echo -e "    ------------------------"      >> $HOME/drwho_results/$file.txt
echo -e "        SOA reverse whois"         >> $HOME/drwho_results/$file.txt
echo -e "    ------------------------\n"    >> $HOME/drwho_results/$file.txt
echo -e "SOA Record Reverse whois Summary\n"  >> $HOME/drwho_results/$file.txt
...
f_textfileSeparator
echo -e "    ----------------------"    >> $HOME/drwho_results/$file.txt
echo -e "       Host Geolocation"       >> $HOME/drwho_results/$file.txt
echo -e "    ----------------------\n"  >> $HOME/drwho_results/$file.txt
address=`echo $host_ip`
echo ''
f_geoIP
...
f_textfileSeparator
echo -e "    --------------------"      >> $HOME/drwho_results/$file.txt
echo -e "       MX Geolocation"         >> $HOME/drwho_results/$file.txt
echo -e "    --------------------\n"    >> $HOME/drwho_results/$file.txt
echo -e "MX Record  Geolocation & AS\n" >> $HOME/drwho_results/$file.txt
...
f_textfileSeparator
echo -e "    --------------------"      >> $HOME/drwho_results/$file.txt
echo -e "       SOA Geolocation"        >> $HOME/drwho_results/$file.txt
echo -e "    --------------------\n"    >> $HOME/drwho_results/$file.txt
address=`echo $soa_ip`
f_geoIP
f_removeDir
...
;;
16)
#******************* 16) CMS website title, description & social media"  **********************
f_solidLong
f_makeNewDir
f_textfileSeparator
echo -e "    -----------------------------------------"   >> $HOME/drwho_results/$file.txt
echo -e "        CMS, Title, Content & Social Media"      >> $HOME/drwho_results/$file.txt
echo -e "    -----------------------------------------\n" >> $HOME/drwho_results/$file.txt
f_guessCMS
echo -e "\n"
f_title
...
;;
17)
#************** 17) dump HTTP - headers ********************
f_solidLong
echo -e "${B}HTTP Headers${D}\n"
f_textfileSeparator
echo -e "    --------------------------"    >> $HOME/drwho_results/$file.txt
echo -e "           HTTP-Headers"           >> $HOME/drwho_results/$file.txt
echo -e "    --------------------------\n"  >> $HOME/drwho_results/$file.txt
curl -s -I -L --max-time 4 $target    | tee -a $HOME/drwho_results/$file.txt
f_solidLong
;;
18)
#************** 18) dump robots.txt - File ********************
f_solidLong
...
;;
21)
#*******************  21) check if unauthorized zone transfers are permitted **********************
f_solidLong
f_makeNewDir
echo -e "${B}Zone Transfer Check${D}\n "
f_textfileSeparator
echo -e "    --------------------------"        >> $HOME/drwho_results/$file.txt
echo -e "       Zone Transfer Check"            >> $HOME/drwho_results/$file.txt
echo -e "    --------------------------\n"      >> $HOME/drwho_results/$file.txt
...
;;
22)
#*******************  22) address block reverse host search **********************
f_solidLong
echo -e "${B}Address Block Reverse Host Search${D}\n\n"
f_textfileSeparator
echo -e "    -------------------------"         >> $HOME/drwho_results/$file.txt
echo -e "       Reverse Host Search"            >> $HOME/drwho_results/$file.txt
echo -e "    -------------------------\n"       >> $HOME/drwho_results/$file.txt
...
;;
23)
#*******************  23) subdomain enumeration via hackertarget.com - API **********************
f_solidLong
echo -e "${B}$target Subdomains${D}\n\n"
f_textfileSeparator
echo -e "    ------------------"        >> $HOME/drwho_results/$file.txt
echo -e "        Subdomains"            >> $HOME/drwho_results/$file.txt
echo -e "    ------------------\n"      >> $HOME/drwho_results/$file.txt
...
;;
24)
#*******************  24) reverse IP lookup via hackertarget.com - API **********************
f_solidLong
echo -e "${B}Reverse IP Lookup${D}\n"
echo -e " ${BDim} $target - $host_ip${D}\n"
f_textfileSeparator
echo -e "    -----------------------"   >> $HOME/drwho_results/$file.txt
echo -e "       Reverse IP Lookup"      >> $HOME/drwho_results/$file.txt
echo -e "    -----------------------\n" >> $HOME/drwho_results/$file.txt
echo -e "\n$target - $host_ip\n"        >> $HOME/drwho_results/$file.txt
...
f_textfileSeparator
echo -e "    -----------------------------"     >> $HOME/drwho_results/$file.txt
echo -e "       HEADERS Summary & TITLE"        >> $HOME/drwho_results/$file.txt
echo -e "    -----------------------------\n"   >> $HOME/drwho_results/$file.txt
f_makeNewDir
error_code=6
curl -sf "${target}" > /dev/null
