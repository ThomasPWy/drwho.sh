...
function f_tracePath  {
    ...
    if [ $answer = "y" ]
        ...
        echo ''
        echo -e "\n\n\n== TRACEPATH RESULTS == \n\n" >> $out
        tracepath -b -m 22 $target | tee -a $out
    else
        f_Dashes
    fi
}
...
echo -e "${B}  >>  Target: $target - $host_ip${D}\n"
    ...
    echo -e "  ${B}15)${D}   certificate information"
    echo -e "  ${B}16)${D}   IP geolocation (ext.API)"
    echo -e "  ${B}17)${D}   HTTP headers / robots.txt / link dump"
    echo -e "  ${B}18)${D}   subdomain enumeration options (ext.APIs)"
    echo -e "  ${B}19)${D}   IPv4 address block host search"
    echo -e "  ${B}20)${D}   reverse IP lookup (ext.API)"
    echo -e "  ${B}21)${D}   AS information (ext. API)"
...
function f_menuIP {
    ...
    echo -e "  ${B}34)${D}   dump HTTP headers"
    echo -e "  ${B}35)${D}   link dump"
    echo -e "  ${B}36)${D}   address block reverse host search"
    echo -e "  ${B}37)${D}   reverse IP lookup (ext. API)"
    echo -e "  ${B}38)${D}   AS information (ext. API)"
    echo -e "  ${B}39)${D}   server response times & tracepath"
    echo -e "  ${B}40)${D}   certificate information"
    f_solidShort
}
...
function f_subEnumOptions {
    ...
    echo -e "  81)  search via hackertarget.com"
    echo -e "  82)  search via crt.sh"
    f_solidShort
}

function f_optionsHostSearch {
        echo -e "${B} address block reverse host search - options\n"
        echo "  91)  A record address block reverse host search ($host_prefix.x)"
        echo "  92)  custom- address block reverse host search"
    f_solidShort
}
...
;;
13)
#***********************  13) host whois & rev. whois summary  **************************
f_makeNewDir
f_solidLong
f_solidLineText
...
;;
15)
#***********************  15) certificate information  & -file **************************
f_makeNewDir
f_solidLong
f_solidLineText
timeout 3 openssl s_client -connect $target:443 -brief 2>$tempdir/ssl_sum2.txt
...
cat $tempdir/ssl_sum2.txt | tr -d '"' | sed 's/ = /: /g' | sed 's/_/ - /g' | tr -d '(' | tr -d ')' |
sed 's/^ *//' > $tempdir/ssl.txt
echo -e "\n${B}Certificates & SSL/TLS Information${D}\n\n"
echo -e "== CERTIFICATES & SSL/TLS INFORMATION ==\n\n" >> $out
...
;;
16)
#************** 16) target IP geolocation via ip-api.co ********************
f_makeNewDir
f_solidLong
f_solidLineText
echo -e "\n${B}Host IP Geolocation${D}\n"
echo -e " ${BDim}$target - $host_ip${D}"
echo -e "== HOST IP GEOLOCATION ==\n"  >> $out
...
;;
20)
#************** 20) reverse IP lookup ********************
f_makeNewDir
f_solidLong
f_solidLineText
echo -e "\n${B}Reverse IP Lookup${D}\n\n"
...
echo -e "${B}Autonomous System Information (via hackertarget.com)${D}\n\n" 
echo -e "== AUTONOMOUS SYSTEM INFORMATION (via hackertarget.com)== \n\n" >> $out
curl -s --max-time 4 https://api.hackertarget.com/aslookup/?q=$host_ip | tee -a $out
echo ''
f_solidLong
f_removeDir
;;
...
f_solidLong
f_solidLineText
echo -e "\n${B}Zone Transfer Check${D}\n\n"
echo -e "== ZONE TRANSFER CHECK ==\n" >> $out
...
;;
33)
#******************* 33) target IP geolocation via ip-api.co  **********************
f_makeNewDir
f_solidLong
f_solidLineText
echo -e "${B}$target geolocation${D}\n"
echo -e "== $target GEOLOCATION ==\n\n" >> $out
address=`echo $target`
f_geoIP | tee -a $out
f_solidLong
f_removeDir
...
;;
40)
#***********************  40) certificate information  & -file **************************
f_makeNewDir
f_solidLong
f_solidLineText
timeout 3 openssl s_client -connect $target:443 -brief 2>$tempdir/ssl_sum2.txt
...
;;
41)
#************** 41) primary name server reverse whois summary ********************
f_makeNewDir
f_solidLong
f_solidLineText
...
;;
42)
#************** 42) "first" mx server reverse whois summary ********************
f_makeNewDir
f_solidLong
f_solidLineText
...
;;
43)
#************** 43) whois lookup summary - custom input ********************
f_makeNewDir
f_solidLong
f_solidLineText
...
;;
44)
#************** 44) reverse whois lookup summary - custom input ********************
f_makeNewDir
f_solidLong
f_solidLineText
...
;;
61)
#************** 61) primary name server geolocation  ********************
f_makeNewDir
f_solidLong
f_solidLineText
soa_ip=`host -t A $soa_url | cut -d ' ' -f 4-`
...
;;
62)
#************** 62) IP geolocation - custom input  ********************
f_makeNewDir
f_solidLong
f_solidLineText
...
;;
81)
#************** 81) subdomain enumeration (via hackertarget.com - API)  ********************
f_makeNewDir
f_solidLong
f_solidLineText
...
;;
82)
#************** 82) subdomain enumeration (via crt.sh - API)  ********************
f_makeNewDir
f_solidLong
f_solidLineText
...
sed '/<TD/d' | sed '/<\/A>/d' | sed '/<TITLE>/d' | sed '/<TH/d' | sort | uniq | tee -a $out
f_solidLong
f_removeDir
;;
91)
#************** 91) domain host address block reverse host search  ********************
f_makeNewDir
f_solidLong
f_solidLineText
...
;;
92)
#************** 92) address block reverse host search - custom input  ********************
f_makeNewDir
f_solidLong
f_solidLineText
f_getPrefixes
...
echo -e "\n"
echo -e -n "Continue? ${BDim} [y] yes | no [any key]${D}   "
    read answer
    if [ $answer = "y" ]
        then
...
read prefx
echo ''
echo -e "\n\n== $prefx ADDRESS BLOCK REVERSE HOST SEARCH ==\n\n" >> $out
f_hostSearch | tee -a $out
fi
f_solidLong
f_removeDir
