...
function f_optionsHostSearch {
    ...
    mx_prefx=`echo $mx_first_ip | rev | cut -d '.' -f 2- | rev`
    ...
    soa_prefix=`echo $soa_ip | rev | cut -d '.' -f 2- | rev`
        ...
        echo "  81)  domain host address block reverse host search"
        echo "  82)  MX record- address block reverse host search - $mx_prefix"
        echo "  83)  SOA- record address block reverse host search - $soa_prefix"
